...
Флаг `MICROPY_HW_ENABLE_RNG` был явно выставлен в ноль — в расчёте на то, что так отключаются обе ветки. Но [проверка в коде](https://github.com/switck/libngu/blob/cf1988aa54969a7d2dcef261ee664a41a7013262/ngu/random.c) была написана через `#ifndef`, то есть смотрела, определён ли макрос, а не равен ли он нулю. Определён — значит, `#error` не сработал, и сборка молча выбрала [программную реализацию](https://github.com/micropython/micropython/blob/master/ports/stm32/rng.c). Обе реализации имели одинаковую сигнатуру, нужный TRNG-код физически присутствовал в бинарнике — ревью это видело и успокаивалось.
...
Затронуты все прошивки Mk3 начиная с 4.0.1 (март 2021) и любые последующие — так формулирует [Coinkite](https://blog.coinkite.com/coldcard-mk3-seed-generation-warning/); [Block](https://engineering.block.xyz/blog/predictable-rng-fallback-and-32-bit-reseed-in-coldcard-firmware) отсчитывает от 4.0.0 (17 марта 2021) и указывает диапазон Mk2/Mk3 4.0.0–4.1.9. Исправленные версии для актуальных моделей — [5.6.0 для Mk4 и Mk5 и 1.5.0Q для Q](https://coldcard.com/downloads). 
{{< /hint >}}
...
Это тот же пример, что приведён [в документации COLDCARD](https://coldcard.com/docs/verifying-dice-roll-math/). Ровно этот метод реализован в COLDCARD, в SeedSigner, в инструменте Иэна Коулмана и в Seed Tool — и именно поэтому их результаты можно сверять между собой.
...
{{< hint info >}} 
Иными словами: в режиме `Dice` предупреждение излишне строгое, а в режиме `Base 10` его отсутствие излишне оптимистично. Но выбирать всё равно надо `Base 10` — не ради циферки на экране, а ради совместимости с остальными реализациями. Только она даёт возможность перекрёстной проверки. 
{{< /hint >}}
...
1. Сгенерируйте на устройстве первый адрес получения и **посмотрите его на экране самого устройства**, а не в приложении на компьютере.
