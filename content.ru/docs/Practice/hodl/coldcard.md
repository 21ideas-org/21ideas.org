...
{{< hint danger >}}
**Обновление, июль 2026.** В прошивках COLDCARD Mk3 начиная с версии 4.0.1 (март 2021) обнаружена уязвимость генератора случайных чисел: при создании сида **без** добавления бросков кости устройство могло использовать программный запасной генератор вместо аппаратного TRNG, что резко снижало реальную энтропию ключа. Подробности и разбор — в [отдельной статье](/dice-seed).

**Практический вывод: шаг 5 выше — добавление энтропии броском кости (пункт 4 на экране) — теперь не опциональный, а обязательный.** Если ваш сид создан на Mk3 без этого шага, считайте его скомпрометированным и переносите средства на новый.

Актуальные прошивки (5.6.0+ для Mk4/Mk5, 1.5.0Q+ для Q) уязвимость устраняют, но правило «добавляйте броски» стоит соблюдать в любом случае — это не требует доверия к генератору устройства.
{{< /hint >}}
