...
Хотя модель One поддерживает множество токенов (а модель T - еще больше), SatoshiLabs предлагает [прошивку](https://trezor.io/learn/a/bitcoin-only-firmware-on-trezor) с поддержкой исключительно биткоина, в которой удалены все неосновные приложения, включая поддержку альткоинов, U2F и Trezor Password Manager.
...
*Model T* обзавелась рядом улучшений по сравнению с *Model One*: сенсорный экран пришел на смену кнопкам, а порт microUSB был заменен на более распространенный USB-C. В *Trezor Model T* также добавлен слот microSD, но он не может использоваться для подписания с воздушным зазором через PSBT (частично подписанные биткоин-транзакции).
...
{{< hint info >}}
\* Ledger позволяет прикрепить PIN-код к кошельку, защищенному парольной фразой
...
| **Производитель** | **Год выпуска** | **Цена** | **Подходит для** | **Нативное приложение** |
|---------------|-------------|------|--------------|---------------------|
| Coinkite | 2022 | [₽26,600](https://sunscrypt.ru/catalog/coinkite/coldcard-mk4/) | Промежуточный | N/A |
...
### NFC-V (коммуникации ближнего поля)
...
Чем выше в списке ваших требований находится понятие "безопасность", тем выше, в вашем списке аппаратных кошельков окажется *Coldcard Mk4*. Множество уникальных функций и элементов дизайна способствуют тому, что устройство пользуется популярностью среди опытных биткоинеров:
...
- **Светодиодные индикаторы безопасности:** Два светодиода расположены рядом с дисплеем устройства. Они подтверждают (зеленый) или предупреждают (красный), что 1) содержимое защищенного элемента не изменилось с момента последнего использования устройства, 2) программное обеспечение устройства не скомпроментировано, или 3) в устройства не было внесено никаких физических изменений с момента его выпуска.
...
{{< hint info >}}
\* Как аппаратное, так и программное обеспечение *Coldcard* полностью доступно для просмотра из исходных кодов, а его архитектура позволяет не доверять защищенному элементу

† Кошелек Coldcard управляется теми же ключами, что и ваш основной кошелек

‡ PIN-код "brick-me" Coldcard не сбрасывает устройство, а уничтожает его
{{< /hint >}}
...
Заметно, что в этом устройстве отсутствует защитный элемент (это намеренно сделано для улучшения безопасности). Вместо этого используется [альтернативная модель](https://help.blockstream.com/hc/en-us/articles/13745404122265-Does-Blockstream-Jade-have-a-secure-element-), позволяющая использовать все аппаратные и программные компоненты с [открытым исходным кодом](https://github.com/Blockstream/Jade).
...
При использовании *Jade* в паре с кошельком Blockstream Green в можете отправлять и получать активов, выпущенные в сети Liquid. Ранее Ledger Nano S также поддерживал некоторые активы Liquid Network, но эта модель была снята с производства, и Jade стал единственным коммерчески доступным вариантом.
...
Произведенный в Швейцарии BitBox 02 - это компактный и незаметный выбор для хранения холода. Его дизайн основан на оригинальной модели BitBox 01, которая была снята с производства в ноябре 2019 года. Однако на этот раз Shift Crypto выпустила версию, поддерживающую только биткоин-прошивку, чтобы уменьшить векторы атак по сравнению с мультитокен-совместимым устройством.

"BitBox02 Bitcoin-only будет иметь только биткойн-прошивку. Ничего другого. В отличие от других аппаратных кошельков, версия Bitcoin-only не может быть перенастроена на поддержку других монет. Прошивка Bitcoin-only блокируется при заводской настройке".

### Shift Crypto

Подтверждение или отклонение действий на устройстве осуществляется прикосновением к датчикам, расположенным по его бокам. Встроенный экран также позволяет вводить PIN-код на устройстве. Основным недостатком BitBox является невозможность использования устройства в защищенном режиме.

### Малый ключ для повышения уровня безопасности

BitBox02, не имеющий явной маркировки (кроме небольшого логотипа Shift Crypto), на первый взгляд кажется обычным черным USB-накопителем. Это делает его идеальным вариантом для тех, кому необходимо использовать аппаратный кошелек в общественных местах.

### Кабели не требуются

Устройство подключается непосредственно к компьютеру или смартфону через порт USB-C (или USB-порт с прилагаемым адаптером), что позволяет обойтись без кабелей. Эта опция будет особенно привлекательна для пользователей, которым устройство может понадобиться для большого количества малозначимых транзакций и для которых удобство важнее конфиденциальности.
...
| **Производитель** | **Год выпуска** | **Цена** | **Подходит для** | **Нативное приложение** |
|---------------|-------------|------|--------------|---------------------|
| Yanssie HK | 2021 | [₽14,250](https://sunscrypt.ru/catalog/keystone/keystone-3-pro/) |  Начинающих | Keystone |
...
Это устройство, ранее известное как *Cobo Vault*, оснащено большим сенсорным экраном, что делает управление им похожим на управление смартфоном. Несмотря на поддержку большого количества токенов, доступна прошивка только для Биткоина (после установки биткоин-онли прошивки вернуть щиткоин-функционал невозможно).

Аппаратный дизайн *Keystone Pro*, как и прошивка защитного элемента, имеют открытый исходный код. Однако код прошивки устройства закрыт, хотя и предлагается независимый аудит кода).

### Датчик отпечатков пальцев

Расположенный на задней панели устройства, датчик может быть активирован для разблокировки и подписания транзакций. Однако решение использовать датчик отпечатков пальцев может оказаться рискованным. Авторизация такого рода уязвима перед так называемыми атаками гаечным ключом и лишает возможности правдоподобного отрицания.

### Съемные аккумуляторные блоки

В *Keystone Pro* предусмотрена возможность использования батарейки типа ААА или перезаряжаемого литий-ионного аккумулятора (оба варианта входят в комплект поставки) для работы в режиме с воздушным зазором. Как и в случае с *Coldcard Mk4*, использование внешних источников питания устраняет угрозу несанкционированной передачи данных по скомпрометированным зарядным кабелям.

### Самоуничтожение при попытке взлома

При попытке доступа к плате путем снятия экрана устройство инициирует процесс самоуничтожения: конфиденциальные данные стираются и устройство превращается в "кирпич". Следует отметить, что эта функция осуществляется благодаря кнопочной батарейке с двухлетним сроком службы, поэтому для продолжения работы этого защитного механизма  необходимо заменить батарею через 2 года.
...
| **Производитель** | **Год выпуска** | **Цена** | **Подходит для** | **Нативное приложение** |
|--------------------|-------------|------|--------------|---------------------|
| Foundation Devices | 2022 | [₽28,500](https://sunscrypt.ru/catalog/foundation/passport-batch/) | Начинающих | Envoy |
...
Первое, что бросается в глаза при виде второй итерации аппаратных кошельков *Passport* от компании *Foundation* - это цена. Намеренно лишенный инструментов беспроводной связи, кошелек получает данные с помощью карты microSD и камеры. Несмотря на наличие порта USB-C, устройство способно только на подзаряжаться через этот порт и не позволяет передавать какие-либо данные (контакты удалены). 
...
SeedSigner - это шаблон для самостоятельной сборки аппаратного кошелька без излишеств, способного выполнять ряд критически важных операций. Основными соображениями при проектировании *SeedSigner* было стремление создать недорогое устройство, не хранящее состояния и состоящее из аппаратных средств общего назначения. Устройство ориентировано на долгосрочных держателей и пользователей мультисиг-схем хранения. 
...
### Устойчивое сообщество сборщиков

Устройство, созданное из широкодоступных деталей, исключает вероятность отказа какой-либо отдельной компании. Все в этом устройстве обладает открытым исходным кодом. По этим и другим причинам устройство привлекает все больше разработчиков, дизайнеров и технарей, предлагающих всевозможные решения и внедряющих дополнительные функции.
