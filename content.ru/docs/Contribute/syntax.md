...
4. `cover` - поле для указания пути к обложке материала. Если у обложки есть отдельная версия для тёмной темы, укажите путь к ней в опцональном атрибуте `coverDark`. Подробнее о добавлении изображений читайте [далее в этой статье](/syntax/#изображения). 
