...
12. Чтобы добавить в статью изображения необходимо загрузить их на репозиторий. Для этого в том же разделе, где мы разместили статью, создаем папку с идентичным названием.
13. Чтобы создать папку в GitHub (пустые папки площадка создавать не позволяет) подготавливаем необходимые изображения, открываем папку, в которой находится статья (в нашем примере это папка "economics") и нажимаем "Add file" -> "Upload files". О стандартах именования изображений можно узнать [здесь](https://new.21ideas.org/docs/Contribute/syntax/#изображения), о том как впоследствии добавлять изображения в статью написано там же.

![upload](img-07.png)

14. Когда статья (и изображения) готова и проверена, нажимаем "Pull requests" -> "New pull request" (может понадобиться подтвердить свои намерения несколько раз, здесь все интуитивно).
...
15.  Во всплывающем окне указываем заголовок пулл-реквеста и описание проделанной работы. Это важный пункт, без которого модераторам будет сложнее понять что конкретно необходимо проверить. Это также необходимо для дальнейшего аудита кода проекта.
...
1. Открываем [репозиторий](https://github.com/21ideas-org/21ideas.org) 21ideas на GitHub, нажимаем кнопку"Code" и иконку копирования, появившуюся в выпадающем окне.
...
4. В правом верхнем углу нажимаем иконку, как указано на скриншоте ниже. Это вызывает окно терминала - рабочего поля, в котором вы впоследствии сможете задавать программе команды. Терминал также можно вызвать сочитанием клавиш "Ctrl+J" на Windows или "Cmd+J" на Mac&
...
6. В терминале прописываем следующую команду:
