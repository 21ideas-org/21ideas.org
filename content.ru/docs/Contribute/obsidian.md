...
- [Hugo](/docs/Contribute/github/#установка-на-windows)
...
Хоть этот способ публикации нельзя назвать **чрезвычайно простым**, всё же его основная и единственная сложность заключается в первичной настройке. Но если вы умеете общаться с компьютером, если не "на ты", то хотя бы уже не "на вы", то вам нечего бояться. Тем более, я постарался раскрыть все мелочи, на которые стоит обратить внимание.

Что бы не перегружать статью информацией, которая уже публиковалась в смежных гидах, я буду просто ссылаться на неё. Например, в этом гиде мы не будем рассматривать процесс установки Hugo, так как об этом уже было написано [здесь](/docs/Contribute/github/#установка-на-windows). 
...
![vault](obsidian_mainpage.png#center)
|:--:|
...
![plugins](obsidian_plugins.png#center)
...
- [Windows](/docs/Contribute/github/#установка-на-windows)
- [MacOS](/docs/Contribute/github/#установка-на-windows)
{{< /hint >}}
...
![button](button_obsidian.png#center)

Иконка между `main` и `Обратных ссылок: 0` нам и нужна. Нажимаем её. Если вы на протяжении всего гида делали всё правильно, то у вас запустится новая область в правой части окна Obsidian. Теперь ваш редактор научился в прямом эфире отображать ваши правки так, как они будут выглядеть в итоговом варианте на сайте 21ideas. Например, вот как выглядело моё окно Obsidian при написании данного гида: 

![workspace](obsidian_workspace.png#center)
...
![sync](/docs/Contribute/github/img-07-1.png#center)

На момент написания моя копия была актуальна, поэтому в качестве примера возьмем скриншоты из гида: [Как добавить статью?](/docs/Contribute/github/)
...
![update](/docs/Contribute/github/img-07-2.png#center)

{{< hint danger >}}
❗️Помните, что в репозиторий постоянно вносятся изменения другими людьми, поэтому важно следить за тем, что ваш форк актуален.

После того, как вы нажали `Sync fork` около удаленной копии, нужно так же подгрузить эти изменения к себе на компьютер командой `git fetch` в терминале, либо командой `Obsidian Git: fetch` в палитре команд Obsidian.
{{< /hint >}}

Убедились, что версия актуальна. Далее открываем вкладку `Pull requests` и жмем `New pull request` (может понадобиться подтвердить свои намерения несколько раз, здесь все интуитивно).

![request](/docs/Contribute/github/img-08.png#center)

![newrequest](/docs/Contribute/github/img-09.png#center)
...
![pull_overview](/docs/Contribute/github/img-10.png#center)
...
Если вы сделали всё правильно, то после первичной настройки Obsidian по данному гиду, в дальнейшем вам останется только обновлять свою версию копии репозитория и отправлять пул-реквесты с новыми статьями и правками. Надеемся как можно скорее увидеть ваши пул-реквест!