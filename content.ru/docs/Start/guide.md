...
* [Coldcard](https://coldcard.com/) (гид по кошельку Coldcard доступен по [этой ссылке](/coldcard)) *оставляю упоминание для исторической честности, на данный момент рекомендовать Coldcard не могу*
...
{{< hint danger >}}
**⚠️ Уязвимость COLDCARD, июль 2026.** В прошивках COLDCARD найден дефект генератора случайных чисел: с 2021 года устройство молча использовало программный запасной генератор вместо аппаратного генератора случайных чисел. Затронуты все модели, кроме Mk1. Прежде чем создавать на нём ключ, прочитайте [предупреждение в гиде](/coldcard) и разбор инцидента в статье «[Сид на костях](/dice-seed)». SeedSigner эта уязвимость не затрагивает.
{{< /hint >}}
