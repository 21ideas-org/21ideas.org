...
По своей сути, `OP_RETURN` — это скриптовый [операционный код](/glossary/#op_code-операционный-код) Биткоина, который позволяет пользователям встраивать небольшой объём произвольных данных в транзакцию. Пометка `OP_RETURN` в выходе транзакции указывает на то, что его доказуемо невозможно потратить (unspendable). Это критически важно, поскольку означает, что данные, связанные с этим выходом, не раздувают набор неизрасходованных выходов транзакций ([UTXO](/glossary/#utxo-unspent-transaction-output) set) — важнейшую базу данных, которую все полные узлы Биткоина должны поддерживать для отслеживания биткоинов, которые могут быть потрачены.
...
{{% image "/img/blog/op_return-wars/opreturn-proposal.webp" %}}
