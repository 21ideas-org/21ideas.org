...
16 Октября 2023 года разработчик Antoine Riard [опубликовал описание](https://lists.linuxfoundation.org/pipermail/bitcoin-dev/2023-October/021999.html) описание атаки на каналы в сети Lightning. У разработчиков было полгода, чтобы минимизировать риски атаки, и в соответствии с процедурой [ответственного раскрытия](https://en.wikipedia.org/wiki/Coordinated_vulnerability_disclosure) детали атаки были полностью раскрыты общественности. В этой публикации мы рассмотрим механизм атаки и её оставшиеся нерешённые аспекты.

Для операторов нод сети Lightning я рекомендую убедиться в том, что версия их ноды обновлена и содержит соответствующие патчи для снижения риска ат
...
Маллет отправляет 1 BTC Маллори, используя Боба как роутящую ноду. Согласно стандартной процедуре, Маллори создает секрет для HTLC (Hashed Timelock Contract). Срок действия контракта между Маллет и Бобом истекает на высоте блока 1020, а между Бобом и Маллори - на 1080. 

{{< expand "Как работает HTLC" "..." >}}
В сети Lightning платежи работают через заключение HTLC контракта по цепочке между отправителем и получателем, и всеми промежуточными нодами. 

Условия контракта на каждом шаге (канале) платежа между Алисой (отправитель) и Бобом (получатель) следующие:
* Боб может получить деньги, если знает секрет, который соответствует фиксированному хешу. Этот секрет называется предобразом хеша или доказательством оплаты.
* Алиса может вернуть деньги, если прошло фиксированное количество блоков с момента подтверждения транзакции с контрактом.
...
Боб видит заключённый с ним HTLC контракт и у него есть возможность либо сообщить секрет Оливии и забрать монету, либо ничего не делать. Допустим, Боб решил завершить сделку и передал секрет Оливии. На уровне протокола это реализуется через специальное сообщение и пересогласование баланса канала. 
...
Для разблокировки HTLC внутри транзакции закрытия потребуется ещё одна транзакция ончейн либо с предобразом хеша, либо используя вариант таймаута после достижения нужной высоты блокчейна. 
