...
16 Октября 2023 года разработчик Antoine Riard [опубликовал описание](https://lists.linuxfoundation.org/pipermail/bitcoin-dev/2023-October/021999.html) описание атаки на каналы в сети Lightning. До этого момента у разработчиков было полгода, чтобы минимизировать риски атаки, и в соответствии с процедурой [ответственного раскрытия](https://en.wikipedia.org/wiki/Coordinated_vulnerability_disclosure) детали атаки были полностью раскрыты общественности 16 Октября. В этой публикации мы рассмотрим механизм атаки и её оставшиеся нерешённые аспекты.
...
В сети Lightning платежи работают через заключение HTLC контракта по цепочке между отправителем и получателем, и всеми промежуточными нодами. Контракт заключается между двумя сторонами, поэтому в примере атаки статьи будет заключено два контракта: один между Маллори (роль Алисы) и Бобом, и между Бобом (роль Алисы) и Маллет (роль Боба). Далее мы разберём работу одного HTLC контракта между ролями Алисы и Бобом.
