...
{{< hint danger >}}
**⚠️ Уязвимость COLDCARD, июль 2026.** Описанный выше кошелёк-обманка на аварийном PIN-коде выводится штатным BIP-85, а тот опирается на генератор случайных чисел, который в прошивках COLDCARD с 2021 года был сломан. Затронуты все модели, кроме Mk1: ключи такого кошелька-обманки предсказуемы. Сама идея аварийного PIN-кода при этом остаётся рабочей — вопрос в конкретной реализации. Подробности — в [предупреждении в гиде](/coldcard) и в статье «[Сид на костях](/dice-seed)».
{{< /hint >}}
